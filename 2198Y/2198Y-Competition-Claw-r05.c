...
while (SensorValue[rArmPot] > 0) // Raise the arm until the potentiometer reads 1200 (travelling height)
	{
	  armDownNoPot(127);
	  pivotUp();
	  forward();
...
while (SensorValue[rArmPot] < 1200) // Raise the arm until the potentiometer reads 1200 (travelling height)
	{
	  armUpNoPot(72);
		backward();
}

while (SensorValue[rArmPot] < 2300) // Raise the arm until the potentiometer reads 1200 (travelling height)
	{
	  armUpNoPot(63);
		backward();
		if (SensorValue[lPivotPot] > 300) {
		pivotUp();
}
else{
	pivotRest();
