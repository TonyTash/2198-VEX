...
task autonomous()
{
	...
	clawStop();
	while(SensorValue[armSensor] < 1500)
	{
		forwardAuto(100);
		up();
	}
	holdArm();
	while(SensorValue[leftWheelsSensor] < 1200 && SensorValue[rightWheelsSensor] < 1200)
	{
		forwardAuto(100);
	}
	stopMoving();
	wait1Msec(500);
	clearEncoders();
	ccwRotationBothSide(25,63);
	stopMoving();
	wait1Msec(200);
	clearEncoders();
	while(SensorValue[leftWheelsSensor] > -100 && SensorValue[rightWheelsSensor] > -100)
	{
		backwardAuto(63);
	}
