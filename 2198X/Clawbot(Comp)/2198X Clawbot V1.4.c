...
/*
changelog: V1.4: change armDown() function to half speed.
								add precise control
								change fowardAuto() and backwardAuto() function, add speed option.
								add safe mode driving, so less chance for the robot to fall when it rises up six bar while moving.
*/
#pragma platform(VEX)
...
void holdClaw()//set the int hold to 1 before use this funciton.
{
	while(hold == 1)
	{
		motor[claw] = 40;
...
task usercontrol()
{
	...
	while (true)
	{
		...
		{
			motor[leftMotor1] = (vexRT[Ch3] + vexRT[Ch4]/2.5);
			motor[leftMotor2] = (vexRT[Ch3] + vexRT[Ch4]/2.5);
			motor[leftMotor3] = (vexRT[Ch3] + vexRT[Ch4]/2.5);
			motor[rightMotor4] = (vexRT[Ch3] - vexRT[Ch4]/2.5);
			motor[rightMotor5] = (vexRT[Ch3] - vexRT[Ch4]/2.5);
			motor[rightMotor6] = (vexRT[Ch3] - vexRT[Ch4]/2.5);
		}
		//save driving mode
		if(SensorValue[arm] < 550)
		{
			if(abs(vexRT[Ch4]) <= 63)
			{
