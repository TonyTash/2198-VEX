...
//    if(rArm == lArm) // If two potentiometers are equal, then follow remote input
//    {
//   		if(vexRT[Btn5D] == 1) // If 5D pressed, go up
//   		{
//   			motor[rArmMotor] = 127;
//   			motor[lArmMotor] = 127;
//   		}
//   		else	if(vexRT[Btn6D] == 1) // If 6D pressed, go down
//   		{
//   			motor[rArmMotor] = -127;
//   			motor[lArmMotor] = -127;
//   		}
//   		else // If none pressed, stay at the same value
//   		{
//   			motor[rArmMotor] = 0;
//   			motor[lArmMotor] = 0;
//   		}
...
//   	else if(lArm > rArm) // if left arm is further then right arm, slow the left arm down
//   	{
//   		if(vexRT[Btn5D] == 1) // Go up if 5d pressed
//   		{
//   			motor[rArmMotor] = 127;
//   			motor[lArmMotor] = (127 - motorspeed*2); // Slow the left motor by motorspeed (absolute difference between sensors)*2 (more sensitive)
//   		}
//   		else if(vexRT[Btn6D] == 1) // Go down if 6d pressed
//   		{
//   			motor[rArmMotor] = -127;
//   			motor[lArmMotor] = (-127 + motorspeed*2); // Slow the left motor by motorspeed (absolute difference between sensors)*2 (more sensitive)
//   		}
...
//   	  }
//    }
//    else if(rArm < lArm) // if right arm is further then the left arm, slow the rught arm down
//    {
//    	if(vexRT[Btn5D] == 1) // Go up if 5d pressed
//   		{
//   			motor[rArmMotor] = (127 - motorspeed*2); // slow right arm motor by the difference * 2
//   			motor[lArmMotor] = 127;
//   		}
